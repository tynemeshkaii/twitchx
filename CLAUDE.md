...
- **index.html** — Single self-contained HTML file with all CSS and JS inline. No external dependencies. Contains the full UI: sidebar (profile, favorites, search), stream grid (glassmorphism cards with shimmer placeholders), toolbar (sort/filter), player bar (quality, watch, status), settings modal, custom context menus, browse view (`#browse-view`), channel profile view (`#channel-view`), and multistream view (`#multistream-view`). Channel view shows avatar, display name, bio, follower count, live badge, follow/unfollow toggle (local favorites), Watch Now button, and VODs/Clips tab stubs. Entry points: "View Channel" in the main-grid context menu, and a profile link on each browse stream card. JS: `showChannelView(login, platform, source)` / `hideChannelView()` / `switchChannelTab()` / `toggleChannelFollow()` / `watchChannelStream()` / `window.onChannelProfile(profile)`. `channelViewSource` tracks whether the view was opened from `'grid'` or `'browse'` so the back button restores the correct prior view. Multistream view (`#multistream-view`) hosts up to 4 `<video>` slots in a CSS grid. `multiState` object tracks: `slots[]` (channel/platform/quality per slot), `chatSlot` (which slot owns the chat panel), `audioFocus` (which slot has unmuted audio), `open` (view visible), `chatVisible`. Key functions: `openMultistreamView()` / `closeMultistreamView()` / `addMultiSlot(channel, platform, quality)` / `removeMultiSlot(idx)` / `setAudioFocus(idx)` / `switchMultiChat(idx)`. Entry point: "Watch in Multistream" in the main-grid context menu. `window.onMultiSlotReady(data)` receives the resolved HLS URL and assigns it to the slot's `<video>` element. Uses diff-based rendering (compares login sets, updates in-place when unchanged). All DOM manipulation uses safe methods (createElement, textContent) — no innerHTML with user data.
...
- **`renderGrid` view guards.** `renderGrid()` returns early when `#browse-view` is visible (`document.getElementById('browse-view').classList.contains('hidden')`) and also when `multiState.open` is true. Both checks prevent the periodic stream poller from restoring `stream-grid`'s inline `style.display` (which has higher specificity than the class rule) while an alternative view is open.
- **Multistream slot activation.** When `onMultiSlotReady` assigns a stream to a slot, the active slot container must be shown with `element.style.display = 'block'` — never with `element.style.display = ''`. Clearing the inline style hands control back to CSS, and `.ms-slot-active { display: none }` is the default, so the slot would remain invisible while audio plays.
- **Multistream chat routing.** `onChatSendResult` resolves the target input with `document.getElementById(multiState.open ? 'ms-chat-input' : 'chat-input')` so failed message text is restored to the correct field. `onChatStatus` gates the send input/button on `status.connected && status.authenticated` (not `connected` alone) to prevent sending before authentication completes. `switchMultiChat(idx)` disables the send input and button immediately at the start of the function so they stay disabled during the chat reconnect.
- **Escape key priority.** In `handleKeydown`, the settings overlay is checked before multistream: settings close first, then multistream view, then context menu, then search dropdown. This matches user expectations — Escape always dismisses the topmost layer.

## Testing

298 unit tests across 20 test files in `tests/`:
...
- **test_multistream.py** — `TwitchXApi.add_multi_slot` (success, HLS resolve failure, missing slot idx, stop_multi cleanup)
...
- **Multistream + WebKit audio-without-video.** WKWebView continues playing audio on `<video>` elements whose parent has `display: none`. If a slot container is hidden by CSS (`.ms-slot-active { display: none }`), audio plays while the screen is black. Always use explicit `style.display = 'block'` to show a slot — never clear the inline style with `style.display = ''`.
- **`add_multi_slot` vs `watch`.** `watch()` gates on `self._live_streams` cache — channels not in the poller cache (e.g., channels added directly to multistream) would fail with "is offline". Use `add_multi_slot(channel, platform, quality, slot_idx)` which calls `resolve_hls_url` directly without the live-cache gate.
- **`favoritesMeta` compound key in multistream context menu.** Context menu "Watch in Multistream" check uses `state.favoritesMeta['platform:' + login]` (compound key), same as the main grid. Never use a bare `login` key — two platforms can have identically-named channels.
