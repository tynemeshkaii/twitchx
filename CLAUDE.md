...
- **twitch.py** — Async Twitch Helix client (`TwitchClient`). Handles client-credentials auth with auto-refresh on expiry/401, rate-limit retry (429 + Ratelimit-Reset header), batching (100 items per request), `asyncio.Lock` to prevent concurrent token refreshes, and `search_channels()` for live channel search via `/search/channels` endpoint.
- **storage.py** — Reads/writes `~/.config/twitchx/config.json`. Merges stored config with `DEFAULT_CONFIG` so missing keys never crash the app. Also manages avatar disk cache (`~/.config/twitchx/avatars/`) with 7-day expiry. Includes one-time migration from `~/.config/streamdeck/` on first load.
...
- **Channel search.** Sidebar entry doubles as a search box — typing triggers `TwitchClient.search_channels()` via background thread with 400ms debounce. Results appear in a dropdown below the entry.
